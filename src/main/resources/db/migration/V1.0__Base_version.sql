...
(
  ...
  `pattern_template` VARCHAR(45) NOT NULL,
...
(
  ...
  `dose`             INT         NOT NULL,
  `status`           VARCHAR(20) NOT NULL COMMENT 'assigned\nprocessing\nexecuted\npartly_executed\ncanseled\n',
  ...
  INDEX `tr_pattern_id_idx` (`pattern_id` ASC) VISIBLE,
  ...
  CONSTRAINT `tr_remedy_id`
    FOREIGN KEY (`remedy_id`)
      ...
      ON DELETE NO ACTION
      ON UPDATE NO ACTION,
  CONSTRAINT `tr_pattern_id`
    FOREIGN KEY (`pattern_id`)
      REFERENCES `reha`.`Pattern` (`pattern_id`)
      ON DELETE NO ACTION
      ON UPDATE NO ACTION
)
  ENGINE = InnoDB;
